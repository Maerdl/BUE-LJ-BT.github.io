...
required int32 canLFall = 5;
    ...
    protobuf.load(proto, function (err, root) {
        ...
        var AwesomeMessage = root.lookupType("CanOpenBridge.SDO");
        ...
        var errMsg = SDO.verify(payload);
        if (errMsg)
            throw Error(errMsg);

        // Create a new message
        var message = SDO.create(payload); // or use .fromObject if conversion is necessary

        // Encode a message to an Uint8Array (browser) or Buffer (node)
        var buffer = SDO.encode(message).finish();
        // ... do something with buffer

        // Decode an Uint8Array (browser) or Buffer (node) to a message
        var message = SDO.decode(buffer);
        // ... do something with message

        // If the application uses length-delimited buffers, there is also encodeDelimited and decodeDelimited.

        // Maybe convert the message back to a plain object
        var object = SDO.toObject(message, {
            longs: String,
            enums: String,
            bytes: String,
